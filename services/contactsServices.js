...
exports.listContactsService = async () => {
  const data = await fs.readFile(contactsPath);
  return JSON.parse(data);
};

exports.getContactByIdService = async (contactId) => {
  const contacts = await listContactsService();
  const findContact = contacts.find((item) => item.id === contactId);
  return findContact || null;
};

exports.removeContactService = async (contactId) => {
  const contacts = await listContactsService();
...
exports.addContactService = async (data) => {
  const contacts = await listContactsService();
  const newContact = {
    id: nanoid(),
    ...data,
